...
export default function AdminPage({ user }) {
  ...
  useEffect(() => { loadUsers(); if (isSuperAdmin) { loadLogs(); loadRestoreRequests(); loadBackups(); } }, []);
  ...
  const restoreBackup = async (id) => { if (!confirm(`Восстановить бекап #${id}?`)) return; try { setLoading(true); await API.post(`/backup/restore/${id}`, { user_login: user.login }); alert("✅ Восстановлено!"); loadUsers(); loadLogs(); loadBackups(); } catch (e) { alert("Ошибка"); } finally { setLoading(false); } };
  const deleteBackup = async (id) => { if (!confirm("Удалить?")) return; try { await API.delete(`/backup/${id}`); loadBackups(); } catch (e) {} };
  ...
  return (
    <div style={s.c}>
      ...
      {activeTab==='users'&&(<div style={s.tw}>{isSuperAdmin&&<div style={s.toolbar}><label style={s.tl}><input type="checkbox" checked={showPasswords} onChange={e=>setShowPasswords(e.target.checked)} style={s.cb}/> Пароли</label><label style={s.tl}><input type="checkbox" checked={showSuperAdmin} onChange={e=>setShowSuperAdmin(e.target.checked)} style={s.cb}/> Супер-админ</label></div>}<table style={s.tbl}><thead><tr><th style={s.th}>ID</th><th style={s.th}>Логин</th><th style={s.th}>Статус</th>{isSuperAdmin&&showPasswords&&<th style={s.th}>Пароль</th>}<th style={s.th}>Роль</th><th style={s.th}>Дата</th><th style={s.th}>Действия</th></tr></thead><tbody>{filteredUsers.map(u=>{const isSA=isSuperAdminUser(u);return(<tr key={u.id} style={{...s.tr,background:isSA?'rgba(255,136,0,0.1)':'transparent',borderLeft:isSA?'4px solid #ff8800':'4px solid transparent'}}><td style={s.td}>{u.id}{isSA&&<span style={{color:'#ff8800',marginLeft:'5px'}}>⭐</span>}</td><td style={s.td}><b>{u.login}</b>{isSA&&<span style={s.sal}>СУПЕР-АДМИН</span>}</td><td style={s.td}>{u.approved?<span style={s.ap}>✓</span>:<span style={s.pn}>⏳</span>}</td>{isSuperAdmin&&showPasswords&&<td style={s.td}><span style={{color:'#ff6666',fontFamily:'monospace',fontSize:'12px'}}>{u.access_key}</span></td>}<td style={s.td}>{isSA?<span style={s.sar}>⭐ Супер-админ</span>:u.is_admin?<span style={s.adr}>🛡️ Админ</span>:<span style={s.usr}>👤 Пользователь</span>}</td><td style={{...s.td,color:'#888',fontSize:'12px'}}>{u.created_at?new Date(u.created_at).toLocaleString('ru-RU'):'—'}</td><td style={s.td}><div style={{display:'flex',gap:'5px',flexWrap:'wrap'}}>{isSA?<span style={s.pt}>🔒 Защищено</span>:<>{!u.approved&&<button onClick={()=>approveUser(u.id)} style={s.btnA}>✓</button>}{isSuperAdmin&&u.approved&&!u.is_admin&&<button onClick={()=>makeAdmin(u.id)} style={s.btnM}>⬆</button>}{isSuperAdmin&&u.is_admin&&<button onClick={()=>removeAdmin(u.id)} style={s.btnR}>⬇</button>}{isSuperAdmin&&<button onClick={()=>removeUser(u.id)} style={s.btnD}>🗑</button>}</>}</div></td></tr>)})}</tbody></table></div>)}

      {activeTab==='logs'&&isSuperAdmin&&(<div style={s.tw}>{logs.length>0&&<div style={{marginBottom:'10px',textAlign:'right'}}><button onClick={clearAllLogs} style={{background:'#660000',color:'#ff6666',border:'1px solid #ff4444',padding:'8px 16px',borderRadius:'6px',cursor:'pointer'}}>🗑 Очистить все</button></div>}<table style={s.tbl}><thead><tr><th style={s.th}>ID</th><th style={s.th}>Действие</th><th style={s.th}>Дата</th><th style={s.th}></th></tr></thead><tbody>{logs.map(l=>(<tr key={l.id} style={s.tr}><td style={{...s.td,color:'#888'}}>{l.id}</td><td style={s.td}>{l.action}</td><td style={{...s.td,color:'#888',fontSize:'12px'}}>{new Date(l.created_at).toLocaleString('ru-RU')}</td><td style={{...s.td,textAlign:'center'}}><button onClick={()=>deleteLog(l.id)} style={{background:'none',border:'none',color:'#ff6666',cursor:'pointer',fontSize:'16px'}}>✕</button></td></tr>))}</tbody></table></div>)}

      {activeTab==='restore'&&isSuperAdmin&&(<div style={s.tw}><table style={s.tbl}><thead><tr><th style={s.th}>ID</th><th style={s.th}>Пользователь</th><th style={s.th}>Дата</th><th style={s.th}>Статус</th><th style={s.th}>Действия</th></tr></thead><tbody>{restoreRequests.map(req=>(<tr key={req.id} style={s.tr}><td style={s.td}>#{req.id}</td><td style={s.td}>{req.user_login}</td><td style={{...s.td,fontSize:'12px',color:'#888'}}>{new Date(req.created_at).toLocaleString('ru-RU')}</td><td style={s.td}>{req.status==='pending'?<span style={{color:'#ffaa44'}}>⏳</span>:req.status==='approved'?<span style={{color:'#4CAF50'}}>✅</span>:<span style={{color:'#f44336'}}>❌</span>}</td><td style={s.td}>{req.status==='pending'&&<div style={{display:'flex',gap:'6px'}}><button onClick={()=>approveRestore(req.id)} style={{...s.btnA,background:'#4CAF50',fontSize:'12px'}}>✓</button><button onClick={()=>declineRestore(req.id)} style={{...s.btnD,fontSize:'12px'}}>✕</button></div>}</td></tr>))}</tbody></table></div>)}

      {activeTab==='backup'&&(<div>
        {isSuperAdmin&&(<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(350px,1fr))',gap:'20px',marginBottom:'20px'}}><div style={s.bc}><h3 style={s.bt}>📥 Создать бекап</h3><button onClick={createBackup} style={s.bBtn} disabled={loading}>💾 Создать</button></div><div style={s.bc}><h3 style={s.bt}>📤 Восстановить из файла</h3><div style={s.wb}>⚠️ Данные заменятся!</div><input ref={fileInputRef} type="file" accept=".json" onChange={importBackup} style={{display:'none'}}/><button onClick={()=>fileInputRef.current?.click()} style={{...s.bBtn,background:'#aa6600'}} disabled={loading}>📂 Выбрать файл</button></div></div>)}
        {!isSuperAdmin&&(<div style={{maxWidth:'500px',marginBottom:'20px'}}><div style={s.bc}><h3 style={s.bt}>📥 Скачать бекап</h3><button onClick={exportBackup} style={s.bBtn} disabled={loading}>💾 Скачать</button></div></div>)}
        {isSuperAdmin&&(<div style={s.tw}><h3 style={{...s.bt,padding:'15px',margin:0}}>📋 История бекапов ({backups.length})</h3><table style={s.tbl}><thead><tr><th style={s.th}>ID</th><th style={s.th}>Создал</th><th style={s.th}>Размер</th><th style={s.th}>Дата</th><th style={s.th}>Действия</th></tr></thead><tbody>{backups.map(b=>(<tr key={b.id} style={s.tr}><td style={s.td}>#{b.id}</td><td style={s.td}>{b.created_by}</td><td style={s.td}>{formatSize(b.size_bytes)}</td><td style={{...s.td,color:'#888',fontSize:'12px'}}>{new Date(b.created_at).toLocaleString('ru-RU')}</td><td style={s.td}><div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}><button onClick={()=>downloadBackup(b.id)} style={{...s.btnA,background:'#2196F3',fontSize:'12px'}}>📥</button><button onClick={()=>restoreBackup(b.id)} style={{...s.btnA,background:'#FF9800',fontSize:'12px'}}>↩</button><button onClick={()=>deleteBackup(b.id)} style={{...s.btnD,fontSize:'12px'}}>🗑</button></div></td></tr>))}</tbody></table></div>)}
      </div>)}
    </div>
  );
}
...
const s = {
  c:{padding:'20px',height:'100%',color:'#fff',overflow:'auto'},t:{margin:'0 0 20px',fontSize:'24px',fontWeight:'bold'},
  ...
  tabs:{display:'flex',gap:'10px',marginBottom:'15px',flexWrap:'wrap'}, tb:{color:'#fff',border:'none',padding:'10px 20px',borderRadius:'8px',cursor:'pointer',fontSize:'14px',background:'#333'},
  ...
  tl:{color:'#aaa',fontSize:'14px',display:'flex',alignItems:'center',gap:'8px',cursor:'pointer'},cb:{width:'18px',height:'18px'},
  ...
  tr:{borderBottom:'1px solid #3a3a3a'},td:{padding:'12px',borderBottom:'1px solid #3a3a3a',color:'#fff',fontSize:'14px'},
  ...
  bc:{background:'#2a2a2a',borderRadius:'8px',padding:'25px',border:'1px solid #444'}, bt:{color:'#fff',margin:'0 0 15px',fontSize:'18px'},
