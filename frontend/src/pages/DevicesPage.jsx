...
const s = {
  ...
  summary: { cursor: 'pointer', color: '#5a9eff', fontSize: '12px', userSelect: 'none' },
  ...
  btnSm: (bg) => ({ background: bg, color: '#fff', border: 'none', padding: '3px 7px', borderRadius: '3px', cursor: 'pointer', fontSize: '10px', whiteSpace: 'nowrap' }),
};
...
function ComponentTaskModal({ data, onClose, onCreate }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',display:'flex',justifyContent:'center',alignItems:'center',zIndex:1001}}>
      <div style={{background:'#222',borderRadius:'8px',width:'95%',maxWidth:'450px',border:'1px solid #b30000'}}>
        <div style={{display:'flex',justifyContent:'space-between',padding:'15px',borderBottom:'1px solid #333'}}><h3 style={{color:'#fff',margin:0,fontSize:'15px'}}>Собрать составляющую</h3><button onClick={onClose} style={{background:'none',border:'none',color:'#888',fontSize:'18px',cursor:'pointer'}}>X</button></div>
        <div style={{padding:'15px'}}>
          <p style={{color:'#999',fontSize:'12px',marginBottom:'12px'}}>Прибор: <b style={{color:'#fff'}}>{data.device_name}</b><br/>Задача: <b style={{color:'#aa6600'}}>{data.subtask_name}</b></p>
          <div style={{display:'flex',alignItems:'center',gap:'6px',color:'#999',fontSize:'13px'}}><span>Время:</span><input type="number" value={hours} onChange={e=>setHours(Math.max(0,parseInt(e.target.value)||0))} min="0" style={{width:'50px',padding:'6px',background:'#1a1a1a',border:'1px solid #444',borderRadius:'3px',color:'#ccc',textAlign:'center'}}/><span>ч</span><input type="number" value={minutes} onChange={e=>setMinutes(Math.max(0,Math.min(59,parseInt(e.target.value)||0)))} min="0" max="59" style={{width:'50px',padding:'6px',background:'#1a1a1a',border:'1px solid #444',borderRadius:'3px',color:'#ccc',textAlign:'center'}}/><span>мин</span><span style={{color:'#4CAF50',fontSize:'12px',marginLeft:'6px'}}>({Math.floor(tm/60)}ч {tm%60}м)</span></div>
        </div>
        <div style={{display:'flex',gap:'8px',justifyContent:'flex-end',padding:'15px',borderTop:'1px solid #333'}}><button onClick={()=>onCreate(hours,minutes)} style={{background:'#b30000',color:'#fff',border:'none',padding:'8px 16px',borderRadius:'4px',cursor:'pointer',fontSize:'13px'}}>Создать задачу</button><button onClick={onClose} style={{background:'#333',color:'#888',border:'none',padding:'8px 16px',borderRadius:'4px',cursor:'pointer',fontSize:'13px'}}>Отмена</button></div>
      </div>
    </div>
  );
}
...
function QuickTaskModal({ device, onClose, onCreate }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',display:'flex',justifyContent:'center',alignItems:'center',zIndex:1001}}>
      <div style={{background:'#222',borderRadius:'8px',width:'95%',maxWidth:'550px',maxHeight:'80vh',overflow:'auto',border:'1px solid #b30000'}}>
        <div style={{display:'flex',justifyContent:'space-between',padding:'15px',borderBottom:'1px solid #333'}}><h3 style={{color:'#fff',margin:0,fontSize:'15px'}}>Создать задачу: {device.name}</h3><button onClick={onClose} style={{background:'none',border:'none',color:'#888',fontSize:'18px',cursor:'pointer'}}>X</button></div>
        <div style={{padding:'15px'}}>
          {subtasks.map((st, i) => (
            <div key={st.id} style={{background:'#1a1a1a',borderRadius:'6px',padding:'10px',border:'1px solid #333',marginBottom:'8px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px',flexWrap:'wrap',gap:'8px'}}><span style={{color:'#aa6600',fontWeight:'bold',fontSize:'13px'}}>{st.name}</span><div style={{display:'flex',alignItems:'center',gap:'3px',color:'#999',fontSize:'12px'}}><span>Time:</span><input type="number" value={st.hours} onChange={e=>updateH(i,e.target.value)} style={{width:'40px',padding:'4px',background:'#2a2a2a',border:'1px solid #444',borderRadius:'3px',color:'#ccc',textAlign:'center'}} min="0"/><span>h</span><input type="number" value={st.minutes} onChange={e=>updateM(i,e.target.value)} style={{width:'40px',padding:'4px',background:'#2a2a2a',border:'1px solid #444',borderRadius:'3px',color:'#ccc',textAlign:'center'}} min="0" max="59"/><span>m</span></div></div>
              <div style={{display:'flex',flexDirection:'column',gap:'3px',paddingLeft:'8px',borderLeft:'2px solid #333'}}>{st.components.map((c, ci) => (<div key={ci} style={{color:'#999',fontSize:'11px'}}>{c.item_type==='consumable'?'O':'-'} {c.component_name} x{c.quantity} {c.unit}</div>))}</div>
            </div>
          ))}
        </div>
        <div style={{display:'flex',gap:'8px',justifyContent:'flex-end',padding:'15px',borderTop:'1px solid #333'}}><button onClick={handle} style={{background:'#b30000',color:'#fff',border:'none',padding:'8px 16px',borderRadius:'4px',cursor:'pointer',fontSize:'13px'}}>Создать</button><button onClick={onClose} style={{background:'#333',color:'#888',border:'none',padding:'8px 16px',borderRadius:'4px',cursor:'pointer',fontSize:'13px'}}>Отмена</button></div>
      </div>
    </div>
  );
}
...
export default function DevicesPage({ user }) {
  ...
  const load = async () => {
    try { setLoading(true); const [d, i, c] = await Promise.all([API.get("/devices"), API.get("/items"), API.get("/consumables")]); setDevices(d.data||[]); setAllItems(i.data||[]); setAllConsumables(c.data||[]); } catch (e) {} finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);
  ...
  const createCompTask = async (h, m) => {
    const tm = (h||0)*60+(m||0); if (tm<=0) return;
    try {
      const device = devices.find(d => d.id === compData.device_id);
      const comps = [];
      if (device?.items) device.items.forEach(item => { if (item.subtask_name === compData.subtask_name) comps.push({ item_type: item.item_type||(item.item_id?'item':'consumable'), item_id: item.item_id||null, consumable_id: item.consumable_id||null, component_name: item.name||'Компонент', quantity: item.quantity||1, unit: item.unit||'шт.' }); });
      await API.post("/tasks", { device_id: null, device_name: compData.device_name, created_by: user.id, created_by_login: user.login, task_type: 'component', subtasks: [{ name: compData.subtask_name, time_estimate: tm, components: comps }] });
      setShowCompModal(false); setCompData(null);
    } catch (e) {}
  };
  ...
  return (
    <div style={s.wrap}>
      <div style={s.header}><h2 style={s.title}>Приборы</h2><button onClick={addDevice} style={s.btn}>+ Добавить</button></div>
      {loading ? <div style={{textAlign:'center',padding:'20px',color:'#555'}}>Загрузка...</div> :
        <div style={s.tWrap}><table style={s.tbl}><thead><tr><th style={{...s.th,width:'30px'}}>#</th><th style={s.th}>Название</th><th style={{...s.th,width:'50%'}}>Состав</th><th style={{...s.th,width:'180px'}}>Действия</th></tr></thead><tbody>
          {devices.length===0 ? <tr><td colSpan={4} style={s.empty}>Нет приборов</td></tr> :
            devices.map((d, idx) => (
              <tr key={d.id} style={s.tr}>
                        ...
                        {(() => { const g = {}; d.items.forEach(item => { const k = `${item.component_name||''}|||${item.subtask_name||''}`; if(!g[k]) g[k]={comp:item.component_name,sub:item.subtask_name,time:item.time_estimate||240,items:[]}; g[k].items.push(item); });
                          return Object.entries(g).map(([key,data]) => (
                            <div key={key} style={s.sGroup}>
                              <div style={s.sHeader}><span style={s.sName}>{data.sub}</span>{data.time>0&&<span style={s.sTime}>{Math.floor(data.time/60)}ч {data.time%60}м</span>}{isAdmin&&<button onClick={(e)=>{e.stopPropagation();openCompCreator(d,data.comp,data.sub,data.time)}} style={s.btnSm('#1a3a5a')}>Собрать</button>}</div>
                ...
                <td style={s.td}><div style={s.actions}><button onClick={()=>{setCurrent(d);setOpen(true)}} style={s.btnSm('#333')}>Edit</button>{isAdmin&&d.items?.length>0&&<button onClick={()=>openTaskCreator(d)} style={s.btnSm('#b30000')}>Задача</button>}<button onClick={()=>remove(d.id)} style={s.btnSm('#3a1a1a')}>Del</button></div></td>
              </tr>
            ))}
        </tbody></table></div>
      }
