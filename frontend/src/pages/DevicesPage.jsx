    ...
    <div style={s.wrap}>
      ...
      {loading ? <div style={{textAlign:'center',padding:'20px',color:'#555'}}>Загрузка...</div> :
        <div style={s.tWrap}><table style={s.tbl}><thead><tr>
          <th style={{...s.th,width:'30px'}}>#</th><th style={s.th}>Название</th><th style={{...s.th,width:'60px'}}>Себест.</th><th style={{...s.th,width:'45%'}}>Состав</th><th style={{...s.th,width:editMode?'220px':'170px'}}>Действия</th>
        </tr></thead><tbody>
                        ...
                        {(() => { const g = {}; d.items.forEach(item => { const k = `${item.component_name||''}|||${item.subtask_name||''}`; if(!g[k]) g[k]={comp:item.component_name,sub:item.subtask_name,time:item.time_estimate||240,items:[]}; g[k].items.push(item); });
                          return Object.entries(g).map(([key,data]) => (
                            <div key={key} style={s.sGroup}>
                              ...
                              <div style={s.sComps}>{data.items.map((item,i)=>{const st=getStock(item);const sh=st&&(parseFloat(st.quantity)||0)<(parseFloat(item.quantity)||0);return(<div key={i} style={{...s.sItem,background:sh?'rgba(255,0,0,0.08)':'transparent',borderLeft:sh?'2px solid #ff4444':'2px solid transparent',padding:'2px 5px',borderRadius:'2px'}}><span>{item.item_type==='consumable'?'[C]':'[I]'}</span><span style={s.sIName}>{item.name||'Компонент'}</span><span style={s.sIQty}>x{fmt(item.quantity)} {item.unit||'шт.'}</span>{st?<span style={{...s.sStock,color:sh?'#ff4444':'#4CAF50'}}>склад: {fmt(st.quantity)}</span>:<span style={{color:'#ff4444',fontSize:'9px'}}>НЕТ</span>}</div>)})}</div>
                            </div>
                          ));
                        })()}
                      </div>
