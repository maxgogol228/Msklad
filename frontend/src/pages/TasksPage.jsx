...
const styles = {
  ...
  editBtn: { background: '#444', color: '#fff', border: 'none', padding: '5px 10px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' },
  ...
  viewBtn: { background: '#444', color: '#fff', border: 'none', padding: '5px 10px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' },
  startBtn: { background: '#4CAF50', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '5px', cursor: 'pointer', fontSize: '12px' },
  pauseBtn: { background: '#FF9800', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '5px', cursor: 'pointer', fontSize: '12px' },
...
export default function TasksPage({ user }) {
  ...
  const changeTaskStatus = async (tid, st) => {
    try {
      await API.put(`/tasks/${tid}/status`, { status: st, user_id: user.id, user_login: user.login });
      setTasks(p => p.map(t => t.id === tid ? { ...t, status: st } : t));
      if (selectedTask?.id === tid) setSelectedTask(p => ({ ...p, status: st }));
    } catch (e) { alert("Ошибка: " + (e.response?.data?.error || e.message)); }
  };
