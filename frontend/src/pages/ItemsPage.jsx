...
export default function ItemsPage({ user }) {
  ...
  const renderRow = (item, idx) => {
    ...
    return (
      <tr key={item.id} style={{...s.tr, background: item.has_shortage?'rgba(255,0,0,0.08)':low?'rgba(255,0,0,0.04)':'transparent', borderLeft: item.has_shortage?'3px solid #ff4444':low?'3px solid #aa6600':'3px solid transparent'}}>
        ...
        <td style={s.td}><input type="text" inputMode="numeric" value={parseInt(item.quantity)||0} onChange={e=>{const v=parseInt(String(e.target.value).replace(/[^\d-]/g,''))||0;updateField(item.id,'quantity',Math.max(0,v))}} style={s.qInp}/></td>
        ...
        <td style={s.td}>{editMode?<input type="number" value={item.min_quantity||''} onChange={e=>updateField(item.id,'min_quantity',e.target.value===''?null:parseInt(e.target.value)||0)} style={s.inp} placeholder="—"/>:<span style={{color:item.min_quantity?'#999':'#555'}}>{item.min_quantity?`${item.min_quantity} шт.`:'—'}</span>}</td>
  ...
  const cols = editMode ? (sortMode==='shelf'?12:11) : (sortMode==='shelf'?11:10);
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.tWrap}><table style={s.tbl}><thead><tr>
        ...
        <th style={s.th}>Нужно</th><th style={s.th}>Мин.</th><th style={s.th}>Статус</th>
