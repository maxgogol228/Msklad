  ...
  const renderRow = (item, idx) => {
    ...
    return (
      <tr key={item.id} style={{...s.tr, background: shortage?'rgba(255,0,0,0.08)':low?'rgba(255,0,0,0.04)':'transparent', borderLeft: shortage?'3px solid #ff4444':low?'3px solid #aa6600':'3px solid transparent'}}>
        ...
        <td style={s.td}>{editMode?<input value={item.name||''} onChange={e=>updateField(item.id,'name',e.target.value)} style={s.inp}/>:<span style={{color:shortage?'#ff6666':low?'#ffaa44':'#ccc'}}>{item.name}</span>}</td>
        {sortMode==='shelf'&&<td style={s.td}>{editMode?<select value={item.category_id||''} onChange={e=>updateField(item.id,'category_id',e.target.value||null)} style={{...s.inp,cursor:'pointer'}}><option value="">Без категории</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select>:<span style={{color:item.category_id?'#5a9eff':'#555',fontSize:'10px'}}>{getCat(item.category_id)}</span>}</td>}
        ...
        <td style={s.td}><input type="text" inputMode="decimal" value={fmt(item.quantity)} onChange={e=>{const v=parseFloat(String(e.target.value).replace(',','.'));if(!isNaN(v))updateField(item.id,'quantity',v)}} style={s.qInp}/></td>
        <td style={s.td}><div style={s.priceWrap}><input type="text" inputMode="decimal" value={item.price||''} onChange={e=>{const v=e.target.value.replace(/[^\d.]/g,'');updateField(item.id,'price',parseFloat(v)||0)}} style={{...s.inp,width:'50px'}} placeholder="0"/><span style={s.rub}>руб.</span></div></td>
        ...
        <td style={s.td}>{editMode?<select value={item.unit||'шт.'} onChange={e=>updateField(item.id,'unit',e.target.value)} style={{...s.inp,cursor:'pointer'}}>{units.map(u=><option key={u} value={u}>{u}</option>)}</select>:<span style={{color:'#888',fontSize:'10px'}}>{item.unit||'шт.'}</span>}</td>
        ...
        <td style={s.td}>{editMode&&<button onClick={()=>remove(item.id)} style={s.btnSm('#3a1a1a')}>Del</button>}</td>
      </tr>
    );
  };

  const cols = editMode ? (sortMode==='shelf'?13:12) : (sortMode==='shelf'?12:11);
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.tWrap}><table style={s.tbl}><thead><tr>
        ...
        <th style={s.th}>Ед.</th><th style={s.th}>Нужно</th><th style={s.th}>Мин.</th><th style={s.th}>Статус</th>
        {editMode&&<th style={s.th}>Действия</th>}
