...
const s = {
  ...
  btnEditMode: (active) => ({ background: active ? '#b30000' : '#333', color: active ? '#fff' : '#aaa', border: active ? '1px solid #b30000' : '1px solid #555', padding: '5px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', whiteSpace: 'nowrap' }),
  ...
  qInp: { width: '55px', padding: '3px 4px', background: '#1a1a1a', border: '1px solid #444', borderRadius: '3px', color: '#ccc', textAlign: 'center', fontSize: '11px' },
...
export default function ConsumablesPage({ user }) {
  ...
  const saveEdit = async () => { try { await API.put(`/consumables/${editingId}`, {...editData, user_login:user.login}); setEditingId(null); setEditMode(false); load(); } catch (e) {} };
  const cancelEdit = () => { setEditingId(null); setEditMode(false); };
  ...
  const renderRow = (item, idx) => {
    ...
    return (
      <tr key={item.id} style={{...s.tr, background: shortage?'rgba(255,0,0,0.08)':low?'rgba(255,0,0,0.04)':'transparent', borderLeft: shortage?'3px solid #ff4444':low?'3px solid #aa6600':'3px solid transparent'}}>
        ...
        <td style={s.td}>
          {editing ? <input value={editData.name||''} onChange={e=>setEditData({...editData,name:e.target.value})} style={s.inp} autoFocus/> :
          editMode ? <input value={item.name||''} onChange={e=>updateField(item.id,'name',e.target.value)} style={s.inp}/> :
          <span style={{color:shortage?'#ff6666':low?'#ffaa44':'#ccc'}}>{item.name}</span>}
        </td>
        {sortMode==='shelf'&&(
          <td style={s.td}>
            {editing ? <select value={editData.category_id||''} onChange={e=>setEditData({...editData,category_id:e.target.value||null})} style={{...s.inp,cursor:'pointer'}}><option value="">Без категории</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select> :
            editMode ? <select value={item.category_id||''} onChange={e=>updateField(item.id,'category_id',e.target.value||null)} style={{...s.inp,cursor:'pointer'}}><option value="">Без категории</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select> :
            <span style={{color:item.category_id?'#5a9eff':'#555',fontSize:'10px'}}>{getCat(item.category_id)}</span>}
          </td>
        )}
        ...
        <td style={s.td}>{editing ? <input type="text" value={editData.quantity||0} onChange={e=>{const v=e.target.value.replace(',','.');const n=parseFloat(v);setEditData({...editData,quantity:isNaN(n)?e.target.value:n})}} style={s.inp}/> : <input type="text" inputMode="decimal" value={fmt(item.quantity)} onChange={e=>updateQty(item.id,e.target.value)} style={s.qInp}/>}</td>
        <td style={s.td}><div style={s.priceWrap}>{editing?<input type="text" value={editData.price||''} onChange={e=>{const v=e.target.value.replace(/[^\d.]/g,'');setEditData({...editData,price:parseFloat(v)||0})}} style={{...s.inp,width:'50px'}} placeholder="0"/>:<input type="text" inputMode="decimal" value={item.price||''} onChange={e=>{const v=e.target.value.replace(/[^\d.]/g,'');updateField(item.id,'price',parseFloat(v)||0)}} style={{...s.inp,width:'50px'}} placeholder="0"/>}<span style={s.rub}>руб.</span></div></td>
        <td style={s.td}>{editing?<input type="text" value={editData.price_per||1} onChange={e=>setEditData({...editData,price_per:parseFloat(e.target.value)||0})} style={{...s.inp,width:'38px'}}/>:<input type="text" inputMode="decimal" value={item.price_per||1} onChange={e=>updateField(item.id,'price_per',parseFloat(e.target.value)||0)} style={{...s.inp,width:'38px'}}/>}</td>
        <td style={s.td}>{editing?<select value={editData.unit||'шт.'} onChange={e=>setEditData({...editData,unit:e.target.value})} style={{...s.inp,cursor:'pointer'}}>{units.map(u=><option key={u} value={u}>{u}</option>)}</select>:editMode?<select value={item.unit||'шт.'} onChange={e=>updateField(item.id,'unit',e.target.value)} style={{...s.inp,cursor:'pointer'}}>{units.map(u=><option key={u} value={u}>{u}</option>)}</select>:<span style={{color:'#888',fontSize:'10px'}}>{item.unit||'шт.'}</span>}</td>
        ...
        <td style={s.td}><div style={{display:'flex',gap:'2px'}}>{editing?<><button onClick={saveEdit} style={s.btnSm('#1a3a1a')}>OK</button><button onClick={cancelEdit} style={s.btnSm('#3a1a1a')}>X</button></>:editMode?<button onClick={()=>remove(item.id)} style={s.btnSm('#3a1a1a')}>Del</button>:<button onClick={()=>{setEditingId(item.id);setEditData({...item});setEditMode(true)}} style={s.btnSm('#333')}>Edit</button>}</div></td>
      </tr>
    );
  };
  ...
  const renderTable = () => {
    ...
    if (sortMode==='category') { const g = {}; categories.forEach(c=>{g[c.id]=[]}); g['_']=[]; filtered.forEach(i=>{const k=i.category_id||'_';if(!g[k])g[k]=[];g[k].push(i)}); return (<>{categories.map(c=>{const ci=g[c.id]||[];if(ci.length===0)return null;return(<Fragment key={c.id}><tr style={s.cRow}><td colSpan={cols} style={s.cCell}><div style={s.cHead}><span style={{color:'#ff4444'}}>{c.name}</span><span style={{...s.cCnt,color:'#555'}}>{ci.length} поз.</span></div></td></tr>{ci.map((item,idx)=>renderRow(item,idx))}</Fragment>)})}{(g['_']?.length>0)&&(<Fragment><tr style={s.cRow}><td colSpan={cols} style={s.cCell}><div style={s.cHead}><span style={{color:'#ff4444'}}>Без категории</span><span style={{...s.cCnt,color:'#555'}}>{g['_'].length} поз.</span></div></td></tr>{g['_'].map((item,idx)=>renderRow(item,idx))}</Fragment>)}</>); }
    else { const g = {}; filtered.forEach(i=>{const k=i.shelf||'Без стеллажа';if(!g[k])g[k]=[];g[k].push(i)}); const keys = Object.keys(g).sort((a,b)=>{if(a==='Без стеллажа')return 1;if(b==='Без стеллажа')return-1;const na=parseInt(a),nb=parseInt(b);if(!isNaN(na)&&!isNaN(nb))return na-nb;return a.localeCompare(b)}); return (<>{keys.map(k=>(<Fragment key={k}><tr style={s.cRow}><td colSpan={cols} style={s.cCell}><div style={s.cHead}><span style={{color:'#5a9eff'}}>Стеллаж: {k}</span><span style={{...s.cCnt,color:'#555'}}>{g[k].length} поз.</span></div></td></tr>{g[k].map((item,idx)=>renderRow(item,idx))}</Fragment>))}</>); }
  };
  ...
  return (
    <div style={s.wrap}>
      <div style={s.header}>
        ...
        <div style={s.btns}><button onClick={()=>setShowCatManager(!showCatManager)} style={s.btn2}>Категории ({categories.length})</button><button onClick={add} style={s.btn1}>+ Добавить</button></div>
      </div>
