...
export default function ArchivePage() {
  ...
  const loadArchive = async () => {
    ...
    try {
      const [itemsRes, consumablesRes, devicesRes] = await Promise.all([
        API.get("/archive/items"),
        API.get("/archive/consumables"),
        API.get("/archive/devices")
      ]);
  ...
  const permanentDelete = async (type, id) => {
    if (!confirm("Окончательно удалить? Это действие нельзя отменить!")) return;
    
    try {
      await API.delete(`/archive/${type}/${id}`);
      alert("Элемент окончательно удалён");
      loadArchive();
    } catch (e) {
  ...
  const getData = () => {
    switch(activeTab) {
      case 'items': return archivedItems;
      case 'consumables': return archivedConsumables;
      case 'devices': return archivedDevices;
      default: return [];
    }
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.tabs}>
        <button 
          ...
          style={{
            ...styles.tab,
            background: activeTab === 'items' ? '#b30000' : '#333'
          }}
        >
          📦 Детали ({archivedItems.length})
        </button>
        <button 
          onClick={() => setActiveTab('consumables')}
          style={{
            ...styles.tab,
            background: activeTab === 'consumables' ? '#b30000' : '#333'
          }}
        >
          🔧 Расходники ({archivedConsumables.length})
        </button>
        <button 
          onClick={() => setActiveTab('devices')}
          style={{
            ...styles.tab,
            background: activeTab === 'devices' ? '#b30000' : '#333'
          }}
        >
          🔬 Приборы ({archivedDevices.length})
        </button>
      </div>

      {loading ? (
        <div style={{ color: '#aaa', textAlign: 'center', padding: '40px' }}>Загрузка...</div>
      ) : data.length === 0 ? (
          ...
          <table style={styles.table}>
            <thead>
              <tr>
                ...
                <th style={styles.th}>Дата удаления</th>
                ...
                <tr key={item.id} style={styles.tr}>
                  ...
                  <td style={styles.td}>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button 
                        onClick={() => restore(typeName, item.id)}
                        style={styles.restoreButton}
                      >
                        ↩ Восстановить
                      </button>
                      <button 
                        onClick={() => permanentDelete(typeName, item.id)}
