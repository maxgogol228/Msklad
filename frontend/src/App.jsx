...
export default function App() {
  ...
  useEffect(() => {
    try { const saved = localStorage.getItem('saved_accounts'); if (saved) setSavedAccounts(JSON.parse(saved)); } catch (e) {}
  }, []);
    ...
    const autoLogin = async () => {
      ...
      if (token && savedUser) {
        try {
          ...
          return;
        } catch (e) { localStorage.removeItem('auth_token'); localStorage.removeItem('user_data'); }
      }
      setChecking(false);
    };
    autoLogin();
  ...
  const handleLogout = () => { localStorage.removeItem('auth_token'); localStorage.removeItem('user_data'); setUser(null); };

  if (checking) return <div style={{ height: "100vh", display: "flex", justifyContent: "center", alignItems: "center", background: "#111", color: "#555", fontSize: "13px" }}>Загрузка...</div>;
      ...
      <div style={s.wrap}>
        <div style={s.card}>
          ...
          {savedAccounts.length > 0 && (
            <div style={s.savedRow}><div style={s.savedTitle}>Сохранённые входы</div>
              {savedAccounts.map(acc => (<button key={acc.login} style={s.savedBtn} onClick={() => setForm({ ...form, login: acc.login })}>{acc.login}<span style={s.savedDel} onClick={(e) => { e.stopPropagation(); removeSaved(acc.login); }}>x</span></button>))}
            </div>
          )}
        </div>
      </div>
